...
onMounted(async () => {
  try {
    ...
    const [MainMenueRes, MainSliderRes, SocialsRes] = await Promise.all([
      useFetch(MainMenueURL, {
        baseURL: apiBase,
      }),
      useFetch(MainSliderURL, {
        baseURL: apiBase,
      }),
      useFetch(SocialsURL, {
        baseURL: apiBase,
    ...
    if (MainMenueRes.data.value) {
      const activeSortedMenus = MainMenueRes.data.value.items
        .filter((item) => item.isActive)
        .sort((a, b) => a.levelOrder - b.levelOrder);
      useMenus().value = activeSortedMenus;
    }
    if (MainSliderRes.data.value) {
      useMainSlider().value = MainSliderRes.data.value.items;
    }
    if (SocialsRes.data.value) {
      useMenusSocial().value = SocialsRes.data.value.items;
    }
  } catch (error) {
