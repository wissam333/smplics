<template>
  <div class="mx-auto my-8 p-6 section-bg light-theme-context">
    <div class="timeline-container" @mouseleave="handleMouseLeaveContainer">
      ...
      <!-- NEW: Outer Animated Line and Bullet -->
      <div class="animated-line-1">
        <div class="moving-bullet-1"></div>
      </div>

      <!-- NEW: Inner Animated Line and Bullet -->
...
// NEW: Radii for the animated lines
...
// --- NEW: Animated Lines and Bullets ---

// Base styles for both animated line containers
.animated-line-1,
.animated-line-2 {
  position: absolute;
  top: 50%;
  ...
  transform: translate(-50%, -50%);
  border-radius: 50%;
  // Use a lighter, subtle dotted line for movement
  border: 1px dotted rgba($accent-color, 0.3);
  z-index: 0;
}

// Line 1: Outer Ring - Slower, Clockwise
.animated-line-1 {
  width: $anim-line-1-radius * 2;
  height: $anim-line-1-radius * 2;
  // The rotation of the container moves the bullet
  animation: spin-clockwise 60s linear infinite;
}

// Line 2: Inner Ring - Faster, Counter-Clockwise
.animated-line-2 {
...
// Base styles for the bullets (placed at the top edge of their container)
.moving-bullet-1,
.moving-bullet-2 {
  width: $bullet-size;
  height: $bullet-size;
  ...
  top: 0;
  left: 50%;
  // Centering the bullet exactly on the line
  transform: translate(-50%, -50%);
...
// --- Positioning Logic (Trigonometry) (Unchanged but adapted to use $timeline-radius) ---
$num-circles: 6;
