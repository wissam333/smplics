...
$primary-color: #1a535c; // Deep Teal (Main Text/Icon Color when NOT active)
$secondary-color: #f0f0f0; // Soft Gray (Circle Base Background)
$accent-color: #007bff; // Standard Blue (Accent/Glow Color)
...
.light-theme-context {
  ...
  color: $text-color-dark; // Default text color
}
...
.timeline-text-display {
  ...
  height: $text-size;
  border-radius: 50%;
  background-color: $light-bg-color; // White background
  ...
  p {
    ...
    &.is-fading {
      opacity: 0; // State during fade out/in
    }
  }
}
...
.timeline-circle {
  ...
  &.is-active {
    ...
    border: 3px solid $light-bg-color;
    color: $light-bg-color; // Icon turns white/light
  }
}
