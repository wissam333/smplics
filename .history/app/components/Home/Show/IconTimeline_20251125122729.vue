<template>
  <div class="mx-auto my-8 p-3 md:p-6 section-bg light-theme-context">
    <div class="timeline-container" @mouseleave="handleMouseLeaveContainer">
      ...
      <!-- Timeline Icon Circles with AOS Animations -->
      <div
...
<script setup>
// Import AOS if not already globally available
import AOS from "aos";
...
.timeline-circle {
  ...
  // Initial state for AOS animation
  opacity: 0;
  transform: scale(0.1) translate(-50%, -50%);
  top: 50% !important;
  left: 50% !important;

  // When AOS animation completes
  &.aos-animate {
    opacity: 1;
...
@for $i from 1 through $num-circles {
  .circle-#{$i} {
    ...
    // When AOS animation completes, position the circle in its final place
    &.aos-animate {
      left: calc(50% + #{$x-offset} - (#{$circle-size} / 2)) !important;
      top: calc(50% + #{$y-offset} - (#{$circle-size} / 2)) !important;
    }
    ...
    @media (max-width: $mobile-breakpoint) {
      ...
      &.aos-animate {
        left: calc(
          50% + #{$x-offset-mobile} - (#{$circle-size-mobile} / 2)
        ) !important;
        top: calc(
          50% + #{$y-offset-mobile} - (#{$circle-size-mobile} / 2)
        ) !important;
      }
    }
    ...
    @media (max-width: $small-mobile-breakpoint) {
      ...
      &.aos-animate {
        left: calc(
          50% + #{$x-offset-small} - (#{$circle-size-small-mobile} / 2)
        ) !important;
        top: calc(
          50% + #{$y-offset-small} - (#{$circle-size-small-mobile} / 2)
        ) !important;
      }
    }
  }
