<template>
  <div class="mx-auto my-8 p-6 section-bg light-theme-context">
    <div class="timeline-container" @mouseleave="handleMouseLeaveContainer">
      ...
      <!-- Outer Animated Line (ROTATING ORBIT) and Bullet (ANIMATED) -->
      <div class="animated-line-1">
        <div class="moving-bullet-1"></div>
      </div>

      <!-- Inner Animated Line (ROTATING ORBIT) and Bullet (ANIMATED) -->
...
// Global rotation applied to the entire component
// NEW: Removed spin-global keyframe definition as it is no longer used.

// Pulsing glow for the nucleus and electrons
@keyframes pulse-glow {
    0%, 100% { 
        box-shadow: 0 0 5px rgba($accent-color, 0.4); 
    }
    50% { 
        box-shadow: 0 0 12px rgba($accent-color, 1), 0 0 20px rgba($accent-color, 0.2); 
    }
}
...
.timeline-container {
  ...
  // PREVIOUSLY HAD: animation: spin-global 180s linear infinite; 
  // NOW REMOVED: The timeline core is static.
}

// --- The Center Text Display Circle (The Pulsing Nucleus) ---
.timeline-text-display {
  position: absolute;
  top: 50%;
  left: 50%;
  
  // Important: Keeps the nucleus centered
  ...
  // Radial gradient for nucleus depth
...
.timeline-line {
  ...
  // Custom 3D tilt for this plane - this is the main ring for the icons
  // Remains static
...
.animated-line-2 {
    ...
    left: 50%;
    border-radius: 50%;
    // The visual ring is here
    border: 1px dotted rgba($accent-color, 0.3); 
    z-index: 0; 
}

// Line 1: Outer Ring - Stronger X-tilt, now spinning slowly (The moving path)
.animated-line-1 {
...
// Line 2: Inner Ring - Y-axis rotation for cross-plane look, now spinning slowly (The moving path)
.animated-line-2 {
...
.moving-bullet-1 {
    // Crucial: transform-origin is set to the distance of the line's radius for rotation around the center
    transform-origin: 50% $anim-line-1-radius; 
    // Combining orbit (spin-clockwise) and pulse animation
    animation: spin-clockwise 20s linear infinite, pulse-glow 1.5s infinite alternate; 
}
...
.moving-bullet-2 {
    ...
    // Combining orbit (spin-counter-clockwise) and pulse animation
    animation: spin-counter-clockwise 15s linear infinite, pulse-glow 1.8s infinite alternate;
}
...
.timeline-circle {
  ...
  &.is-active {
    ...
    // When scaled, ensure the 3D perspective is maintained
