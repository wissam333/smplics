...
.home {
  ...
  .custom-swiper {
    ...
    // 1. Define the colors using Swiper's built-in variables
    // This handles the basic colors penetrating the Shadow DOM
    ...
    // 2. Advanced Styling using ::part
    // This targets the pagination container inside the Shadow DOM
    &::part(pagination) {
      display: flex;
    ...
    // 3. The "Pill" Effect
    // Since we can't easily target specific bullets inside Shadow DOM for width changes via CSS alone,
    // we rely on the variables above for the basics.
    // However, if you want the active bullet to be WIDER (a pill shape),
    // we usually inject styles via JS or accept the standard round shape.
    // Below is a trick that works in many modern browsers to access the bullets if exposed:
