...
<template>
  <div class="col-lg-8 mx-auto my-8 p-6 section-bg dark-theme-context">
    <h2 class="text-center mb-10 text-xl font-semibold text-white">
...
// 1. SCSS Variables - DARK THEME PALETTE
...
$dark-bg-color: #121212; // Pure Dark Background (Standard dark mode BG)
$container-bg-color: #1e1e1e; // Slightly lighter dark background for the component area
...
.dark-theme-context {
  background-color: $dark-bg-color; // Overall page context (optional, but good practice)
  color: $text-color; // Default text color
}

.section-bg {
  background-color: $container-bg-color; // Component background
  border-radius: 1rem;
  box-shadow: 0 0 20px rgba(0, 0, 0, 0.5); // Soft, subtle outer shadow
}
...
.timeline-text-display {
  ...
  height: $text-size;
  border-radius: 50%;
  background-color: $container-bg-color; // Dark background for the display area

  // Use a bright glow for the border against the dark background
  border: 2px solid $accent-color;
  box-shadow: 0 0 10px rgba($accent-color, 0.5),
    inset 0 0 5px rgba($accent-color, 0.2);
...
.timeline-circle {
  ...
  justify-content: center;
  align-items: center;
  color: $text-color; // Light icons on dark background
  font-size: 1.75rem;
  cursor: pointer;
  position: absolute;
  z-index: 2;
  ...
  &.is-active {
    ...
    color: $dark-bg-color; // Dark icon inside bright circle
  }
}
