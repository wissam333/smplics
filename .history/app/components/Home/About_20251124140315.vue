...
.about-section {
  ...
  .about-content {
    ...
    .title {
      ...
      .text-secondary-bg {
        padding: 5px 10px;
        background-color: rgba($main, 0.1); // Light tint of main color
        border-radius: 4px;
        color: $text-color;
      }
      .text-main {
  ...
  .cta-button {
    ...
    box-shadow: 0 5px 15px rgba($main, 0.3);

    &:hover {
      background-color: $main;
      transform: translateY(-2px);
      box-shadow: 0 8px 20px rgba($main, 0.4);
    }
  }
  ...
  .about-image-stack {
    position: relative;
    padding: 0px 30px; // Space for the pseudo-element decoration
