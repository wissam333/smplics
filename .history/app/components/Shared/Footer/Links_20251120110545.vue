...
// 1. Use AsyncData for better SSR handling and caching
// Using a unique key 'footer-menus' ensures hydration matches
const { data: menusData } = await useAsyncData("footer-menus", () =>
  useGetSiteApi().GetAll(`${api.MenusMasterGetByCategoryApi}?categoryId=2`)
);

// 2. Computed property for language check (Cleaner template)
const isArabic = computed(() => locale.value === "ar");

// 3. Filter Logic: Handle "Drop Down Menu" check here, not in HTML
// Also handles the case where menusData might be null/undefined
const usefulLinks = computed(() => {
  const items = menusData.value?.items || [];
  return items.filter((item) => item.type !== "Drop Down Menu");
});

// 4. Helper for name localization
