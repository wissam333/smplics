<template>
  <div class="contact-wrapper" data-aos="fade-left">
    <h4 class="section-title" :class="{ ar: isArabic }">
      {{ isArabic ? "الاتصال بنا" : "Contact Us" }}
    </h4>

    <div class="contact-list">
      <div class="contact-item" v-if="contactData.mobile">
        <div class="icon-box"><Icon name="mdi:phone" /></div>
        <div class="info-text ltr-force">{{ contactData.mobile }}</div>
      </div>

      <div class="contact-item" v-if="contactData.mobile2">
        <div class="icon-box"><Icon name="mdi:phone" /></div>
        <div class="info-text ltr-force">{{ contactData.mobile2 }}</div>
      </div>

      <div class="contact-item" v-if="contactData.email">
        <div class="icon-box"><Icon name="mdi:envelope" /></div>
        <div class="info-text">{{ contactData.email }}</div>
      </div>

      <div class="contact-item" v-if="contactData.location">
        <div class="icon-box"><Icon name="mdi:map-marker" /></div>
        <div class="info-text">{{ contactData.location }}</div>
      </div>

      <div class="contact-item" v-if="contactData.website">
        <div class="icon-box"><Icon name="mdi:web" /></div>
        <div class="info-text">{{ contactData.website }}</div>
      </div>
    </div>
  </div>
</template>
  ...
  try {
    ...
    if (paragraphs.length >= 7) {
      contactData.value = {
        ...
        mobile: paragraphs[3],
        mobile2: paragraphs[4],
        website: paragraphs[5],
        email: paragraphs[6],
      };
    }
  } catch (e) {
