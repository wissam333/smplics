...
const checkWindowSize = () => {
  mobile.value = window.matchMedia("(max-width: 991px)").matches;
};
...
.top {
  ...
  @media (max-width: 991px) {
    ...
    &:not(.scrolled) {
      top: 64px; // Slightly higher for mobile if contact info takes more space
    }
  }
    @media (max-width: 768px) {

    &:not(.scrolled) {
      top: 75px; // Slightly higher for mobile if contact info takes more space
    }
  }
}
