...
const { locale } = useI18n(); // Assuming you removed this, but it's often needed elsewhere
...
onMounted(() => {
  ...
  // 2. Check window size immediately (only depends on window object, not scroll timing)
  checkWindowSize();
  
  // 🚨 FINAL FIX: Check the scroll state *after* hydration is complete.
  // nextTick ensures the DOM is fully attached before we read window.scrollY and update reactive state.
  nextTick(() => {
      handleScroll(); 
  });
